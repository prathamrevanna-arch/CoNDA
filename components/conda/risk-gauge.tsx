...
import { riskBand, RISK_COLORS, RISK_LABELS } from "@/lib/conda-data"

type Props = {
  value: number
}

// Semicircular arc gauge. Value 0..100 maps to a 180° sweep (left→right).
...
export function RiskGauge({ value }: Props) {
  ...
  const cy = 130
  const r = 100
  // 180° (left) → 360°/0° (right). Value angle:
  const angle = 180 + (value / 100) * 180
  const cap = polar(cx, cy, r, angle)

  // Band boundaries on the arc.
  const bandStops = [
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* threshold legend: low / medium / high */}
